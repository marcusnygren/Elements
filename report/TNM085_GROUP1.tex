...
%		Målet med projektet har varit att utmanas tekniskt, kombinera medieteknik och datateknik, erfara modellering och simulering, och genom projektet bidra till ökad kunskap hos andra.
	...
	\Subsection{Konservering av energi}
		 RTT säger att förändringen av en fysisk egenskap $ \psi  $ (antingen skalär- eller vektor-baserad) definerad på volymen $ \Omega $ måste vara lika med vad som är förändrat genom randen till $ \Omega $ plus vad som är förändrat innuti $ \Omega $ genom eventuella källor och sänkor. Detta i enlighet med ekvation $ \ref{eq:Reynolds Transport Theorem} $ för att bevara energin.
	


	\NewEnviron{Reynolds Transport Theorem}{%
	\begin{equation}\label{eq:Reynolds Transport Theorem}\begin{split}
	  \BODY
	\end{split}
	\end{equation}
	}

	\begin{Reynolds Transport Theorem}
		...
		\int_{\Omega}^{ } \left ( \frac{  \partial }{\partial t} \psi\ 
				...
				= 
				0	\ \ 	\ \ 	\ \ 	
	\end{Reynolds Transport Theorem}
...
\fi
	...
	Eftersom integralen i ekvation $ \ref{eq:Reynolds Transport Theorem} $ ska vara lika med noll för alla volymer $ \Omega $ erhålls ekvation $ \ref{eq:Reynolds Transport Theorem Leibniz 2} $ som blir det slutgiltliga sambandet för konservering av energi. Detta samband nyttjas i framtagandet av Navier-Stokes ekvationer genom att appliciera det på konserverig av massa och konservering av rörelsemängd.
	...
	I ekvation $ \ref{eq:Konservering av rörelsemängd 3} $ i vänsterledet innehåller den första termen sambandet om konservering av massa, ekvation $ \ref{eq:Konservering av massa 1} $, då
	$ \bold u \cdot \nabla \varrho + \varrho \nabla \cdot \bold u 
...
Sedan beräknades divergensen av hastigheten, se högerledet i ekvation~\ref{eq:Poissons ekvation}. Detta beräknades med central differensen av alla omkringliggande värden ur hastgihets-texturen. Innan beräkningen utfördes kontrollerades om någon av de omkringliggande värde befann sig i ett hinder. Om så var fallet sattes hastigheten där till noll.
